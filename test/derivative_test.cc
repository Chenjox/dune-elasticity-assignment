...
Dune::FieldMatrix<double, 2, 2> rightCauchyToDeformation(const Dune::FieldMatrix<double, 2, 2>& rightCauchy) {
            ...
            // S = 2 psi/C \implies S_ij = 2 * p psi/ p C_ij
                for (int m = 0; m < 4; m++) {
                  ...
                  if (std::abs(SecondPK[entryi][entryj] - centralDifference) > 10.0 || std::abs(SecondPK[entryi][entryj] - forwardDifference) > 10.0 || std::abs(SecondPK[entryi][entryj] - backwardDifference) > 10.0 ){
                    ...
                    // Energys
                    std::cerr << "Actual Energy: " << std::endl << middleEnergy << std::endl;
                    std::cerr << "Actual Energy+eps: " << std::endl << forwardEnergy << std::endl;
                    std::cerr << "Actual Energy-eps: " << std::endl << backwardEnergy << std::endl;
