...
double difference(const Dune::FieldMatrix<double, 2, 2>& A,const Dune::FieldMatrix<double, 2, 2>& B) {
    return std::abs(A[0][0] - B[0][0])
    +std::abs(A[0][1] - B[0][1])
    +std::abs(A[1][0] - B[1][0])
    +std::abs(A[1][1] - B[1][1]);
}
...
int main (int argc, char** argv)
{
    ...
    if (jacobian > 0) {
        ...
        // Pushforward...
        auto rightCauchyIdentity = inverseDeformation.transposed() * rightCauchyGreen * inverseDeformation;
        ...
        auto leftCauchyIdentity = inverseDeformation * leftCauchyGreen * inverseDeformation.transposed();
